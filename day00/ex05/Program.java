...
public class Program {
        ...
        for (int i = 0; i < DAYS_IN_MONTH; i++) {
            for (int j = 0; j < FINISH_HOUR - START_HOUR; j++) {
                if (timetable[i][j][0] != 0) {
                    index = i % DAYS_IN_WEEK + FIRST_DAY_INDEX == 7 ? 0 : i % DAYS_IN_WEEK + FIRST_DAY_INDEX;
    ...
    public static void main(String[] args) {
        ...
        while (!line.equals(".")) {
            if (line.length() <= MAX_NAME_LENGTH && arrayOfStudents.length < MAX_NUMBER_OF_STUDENTS) {
        ...
        int[][][] timetable = new int[DAYS_IN_MONTH][FINISH_HOUR - START_HOUR][arrayOfStudents.length];
        ...
        while (scanner.hasNextInt()) {
            ...
            if (day < 0 || hour < START_HOUR || hour > FINISH_HOUR - 1 || countClasses > MAX_TOTAL_CLASSES_PER_WEEK) {
                System.err.println("IllegalArgument");
                System.exit(-1);
            }

            for (int i = 0; i < arrayOfStudents.length; i++) {
                for (int j = day; j <= DAYS_IN_MONTH; j += 7) {
